...
export default function RoomsEdit({ values }: { values: Values }){
    const route = useRoute();

    const { flash } = usePage<PageProps>().props;
    const { data, setData, post, processing, errors } = useForm<FormValues>({
        bed_type: values.room.room_type_id ?? '',
        room_number: values.room.room_number ?? '',
        status: values.room.status ?? '',
    });

    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();

        post(route('portal.rooms.edit', values.room.id))
    };
        ...
        <MainLayout>
                    ...
                    <Flex justify={{ base: "flex-start", md: "space-between" }} align="center" direction={{ base: "column", md: "row" }}>
                        ...
                        <Breadcrumb.Root>
                            <Breadcrumb.List>
                                ...
                                <Breadcrumb.Separator />
                                <Breadcrumb.Item>
                                    <Breadcrumb.CurrentLink>Edit Room</Breadcrumb.CurrentLink>
                                </Breadcrumb.Item>
                            </Breadcrumb.List>
                        </Breadcrumb.Root>
                    </Flex>
            ...
            <Container maxW="3xl" mt={4} style={{padding: "0px"}}>
                <form onSubmit={handleSubmit} style={{ width: "100%" }}>
                    <Card.Root size="sm">
                        <Card.Header pb={2}>
                            <Heading _dark={{ color: "gray.300" }} color="gray.700" size="md">Edit room form</Heading>
