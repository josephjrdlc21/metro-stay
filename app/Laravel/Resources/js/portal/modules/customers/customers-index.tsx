...
export default function CustomersIndex({ values }: { values: Values }){
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        get(route('portal.customers.index'));
    };

    const handleUpdateStatus = (id: number) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "You want to update the status of this customer.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                router.get(route('portal.customers.update_status', id));
            }
        });
    }
    ...
    const handleUpdatePassword = (id: number) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "You want reset the password of this customer.",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                router.get(route('portal.customers.update_password', id));
            }
        });
    }
                    ...
                    <Flex justify={{ base: "flex-start", md: "space-between" }} align="center" direction={{ base: "column", md: "row" }}>
                        ...
                        <Breadcrumb.Root>
                            <Breadcrumb.List>
                                ...
                                <Breadcrumb.Separator />
                                <Breadcrumb.Item>
                                    <Breadcrumb.CurrentLink>Customers</Breadcrumb.CurrentLink>
                                </Breadcrumb.Item>
                            </Breadcrumb.List>
                        </Breadcrumb.Root>
                    </Flex>
                ...
                <Card.Body>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <HStack mb={2}>
                            ...
                            <Link href={route('portal.customers.index')} disabled={processing}>
                    ...
                    <Flex justify="space-between" align="center" mb={3}>
                        <Heading _dark={{ color: "gray.300" }} color="gray.700" size="lg">Customer Records</Heading>
                        <HStack>
                        </HStack>
                    </Flex>
                        ...
                        <Table.Root variant="outline">
                            <Table.Header>
                                <Table.Row>
                                    ...
                                    <Table.ColumnHeader minW="200px">Email/Contact</Table.ColumnHeader>
                            ...
                            <Table.Body>
                                {values?.record?.data && values?.record?.data.length > 0 ? (
                                    (values?.record?.data.map)(customer => (
                                        <Table.Row key={customer.id}>
                                            <Table.Cell>
                                                <Text fontSize="sm" color="blue.500">{formatId(customer.id)}</Text>
                                                {customer.name}
                                            </Table.Cell>
                                            <Table.Cell>
                                                {customer.email}
                                                <Text textStyle="xs">{customer.phone_number}</Text>
                                            </Table.Cell>
                                            <Table.Cell>
                                                <Status.Root colorPalette={statusBadgeClass(customer.status)}>
                                                    <Status.Indicator />
                                                    {customer.status}
                                                </Status.Root>
                                            </Table.Cell>
                                            <Table.Cell>{dateTime(customer.created_at)}</Table.Cell>
                                                        ...
                                                        <Menu.Positioner>
                                                            <Menu.Content>
                                                                <Menu.Item cursor="pointer" value="show">
                                                                    <Link href={route('portal.customers.show', customer.id)} style={{ border: "0px", outline: "none", boxShadow: "none", textDecoration: "none", color: "inherit"}}>
                                                                        Show Details
                                                                    </Link>
                                                                </Menu.Item>
                                                                <Menu.Item cursor="pointer" value="status" onClick={() => {
                                                                    setTimeout(() => handleUpdateStatus(customer.id), 0);
                                                                }}>
                                                                    {customer.status == "active" ? "Deactivate Account" : "Activate Account"}
                                                                </Menu.Item>
                                                                <Menu.Item cursor="pointer" value="reset" onClick={() => {
                                                                    setTimeout(() => handleUpdatePassword(customer.id), 0);
                                                                }}>
                                                                    Reset Password
                                                                </Menu.Item>
                                                            </Menu.Content>
