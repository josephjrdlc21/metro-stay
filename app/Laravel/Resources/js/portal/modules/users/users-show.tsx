...
export default function UsersShow({ values }: { values: Values }){
                    ...
                    <Flex justify={{ base: "flex-start", md: "space-between" }} align="center" direction={{ base: "column", md: "row" }}>
                        ...
                        <Breadcrumb.Root>
                            <Breadcrumb.List>
                                ...
                                <Breadcrumb.Separator />
                                <Breadcrumb.Item>
                                    <Breadcrumb.CurrentLink>Show User</Breadcrumb.CurrentLink>
                                </Breadcrumb.Item>
                            </Breadcrumb.List>
                        </Breadcrumb.Root>
                    </Flex>
                    ...
                    <Card.Body>
                        <Flex justify="center" mb={6}>
                            <Box h="100px" w="100px">
                                <Avatar.Root size="full">
                                    <Avatar.Fallback name={values.user.name} />
                                    <Avatar.Image src={`${values.user.directory}/${values.user.filename}`}/>
                                </Avatar.Root>
                            </Box>
                        </Flex>
                        <SimpleGrid columns={{ base: 1, md: 2 }} gap={2}>
                            <Box textAlign={{ base: "center", lg: "left" }}>
                                ...
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">{formatId(values.user.id)}</Text>

                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold" mt={4}>Role</Text>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">Master Admin</Text>

                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold" mt={4}>Created At</Text>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">{dateTime(values.user.created_at)}</Text>

                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold" mt={4}>Update At</Text>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">{dateTime(values.user.updated_at)}</Text>
                            </Box>
                            <Box textAlign={{ base: "center", lg: "left" }}>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold">Name</Text>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">{values.user.name}</Text>

                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold" mt={4}>Email</Text>
                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm">{values.user.email}</Text>

                                <Text _dark={{ color: "gray.300" }} color="gray.700" textStyle="sm" fontWeight="semibold" mt={4}>Status</Text>
                                <Status.Root colorPalette={statusBadgeClass(values.user.status)}>
                                    <Status.Indicator />
                                    {values.user.status}
                                </Status.Root>
                            </Box>
